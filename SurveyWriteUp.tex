...
The likely source fo this disctinction is that no one admited to citing data informally in the body of the text; otherwise responses were roughly similar.
...
The idea that ``data use in its own righ provides a form of review''\cite{parsons_data_2010} is frequently put forward in disucssions of data publication, but successful reuse rated below both peer review and ``basis of a traditional paper'' as a means of establishing trust in a dataset.
...
The seeming disconnect could be the result of different ideas about what consitiutes successful reuse vs. where a citation comes from, or the difference between trustworthiness and impact.
...
Formal citation was mobth the most common response to how a dataset creator should be credited (83\%) and to how the respondent actually credited data creators (63\%).
Furthermore, no one admitted to informally citing data in the text of a paper.
It may be that our respondents were particularly concientious, that significant forms of credit are easier to remember, or that researchers are opperating on somewhat different definitions of what constitutes a datset or reuse than the authors of the surveys based on examining papers.
...
The Wellcome Trust report found that 71\% of their repondents already tracked use of their datasets ``through details of publications generated using the data.''\cite{bobrow_establishing_2014} 
...
On the other hand, appreciation of altmetics was quite low; few gave them much weight, but a majority gave them a litle.
...
There is certainly no harm in providing altmerics for datasets, but in the short term, they are unlikely to add much credibility.
...
We make no judgment here about whether the expectations of the unsatisfied reasearchers are reasonable or not, but we did not find any significant differences between satisfied and unsatisfied respondents in the view of how a dataset should be credited.
