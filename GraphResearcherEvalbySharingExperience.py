...
dvar_leikert_responses = responses.reindex(columns=DVAR_RESPONSES)


counts = {}
for column in DVAR_RESPONSES:
  counts[column] = dvar_leikert_responses[column].value_counts()
  
dvar_leikert_responses = pd.DataFrame(counts)
dvar_leikert_responses = dvar_leikert_responses.transpose()
        
merged_responses = pd.merge(ivar_checkbox_responses, dvar_leikert_responses,
