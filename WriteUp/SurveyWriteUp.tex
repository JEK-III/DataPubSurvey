...
However, $0.17$ is comfortably a ``small'' effect, so we are unlikely here to have missed large or even moderate effects by chance\cite{cohen_statisctical_1988}.
...
Statistical significance was tested using Fisher's exact test where possible (i.e., for 2x2 tables) and contingency $\chi^{2}$ in all other cases\cite{agresti_statistical_1992}.
...
Odds Ratios (ORs)\cite{szumilas_explaining_2010} and Fisher's exact test p-values were used to assess the relationships between items depicted in Figure \ref{fig:definition_web}; to enable meaningful comparison, all ORs were calculated in the direction that yeilded a value $\geq1$.

The robustness of all significant results was confirmed using a jacknife procedure in which the test was repeated systematically with each respondent removed; in no cases did the absence of a single respondent raise the p-value above the designated significance threshold.
