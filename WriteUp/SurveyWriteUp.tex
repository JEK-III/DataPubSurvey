...
The reproducibility problem plaguing science in the scholarly \cite{ioannidis_why_2005, prinz_believe_2011, mobley_survey_2013} and mainstream \cite{zimmer_rise_2012, editors_how_2013, editors_trouble_2013} press could be addressed, in part, by opening underlying data to scrutiny \cite{drew_lost_2013, collins_nih_2014}.
...
Data publication appropriates familiar terminology (``publication,'' ``peer review'') from the scholarly literature in order to insinuate data into the existing academic reward system \cite{costello_motivating_2009, lawrence_citation_2011, atici_other_2012}.
...
Data papers are invariably peer-reviewed based on the dataset; its description; and whether the two form a complete, consistent, and useable package \cite{lawrence_citation_2011}.
...
For significance testing, we consolidated sub-disciplines into eight high-level disciplines.
Given the number of respondents, this survey should have 80\% power to detect small effects by chi square ($\chi^{2}$) test (size $\Phi_{C}=0.17$) and 95\% power to detect medium-small effects ($\Phi_{C}=0.22$)\cite{cohen_statistical_1988}.
...
The most common expectations relate to access: 68\% ($n=166$) expect a published dataset to be openly available and 54\% ($n=133$) expect it to be in a repository or database.
...
Only a 29\% minority ($n=70$) expected published data to have been peer-reviewed.
...
We distilled a multiplicity of data publications formats to four generic models–- with or without a data paper and with or without peer review-- and asked respondents how much ``weight'' each would contribute to a researcher's curriculum vit{\ae} (Figure \ref{fig:values}D).
...
For this question, which explicitly addressed evaluation of dataset creators, we were particularly interested in the 26\% (59) of survey respondents who had experience on a tenure and promotions committee (Figure \ref{fig:values}A).
We compared their responses to each feature with those who had not served on a committee by $\chi^{2}$, but found no significant relationships (corrected $\alpha= 0.01$, $\chi^{2}\leq 8.39$, $p\geq 0.078$).
\section*{Discussion}
...
Whereas DataONE uncovered statistically distinct data sharing attitudes between respondents in different disciplines, we generally did not.
...
This survey should have 80\% sensitivity to an effect size at the top of this range, $\Phi_{C}=0.17$, so we find it plausible that the detection of statistically meaningful distinctions in one survey and not the other could be an artifact of the difference in statistical power (from an $n$ of $1329$ vs. $249$) rather than a reflection of real differences in the respondent populations.
However, $0.17$ is comfortably a ``small'' effect\cite{cohen_statistical_1988}, so we are unlikely here to have missed large or even moderate effects by chance.
...
Assessment of plausibility was linked with three other assessment that require domain knowledge: method evaluation, adequacy of metadata for replication, and potential novelty/impact.
...
It is worth nothing that serving as the basis of a research paper by the dataset creator is itself evidence of successful use, albeit not by a third party.
However, respondents did consider citations to be the most useful metric for assessing value/impact.
This apparent contradiction could stem from evaluating trustworthiness and impact differently or from different concepts of  ``successful'' reuse and reuse that that results in a citation.
...
A followup 17 years later found that only 17\% of papers met even this low standard, showing that practice has not improved \cite{mooney_anatomy_2012}.
...
Questions that related to US policies were analyzed based on US respondents only; one questions about the NIH was analyzed based only on US biologists (Figure \ref{fig:policy_knowledge}).
...
To facilitate comparison, $\chi^{2}$ effects sizes were standardized as Cram\'er's V ($\Phi_{C}$)\cite{cramer_mathematical_1999}.
...
An IPython notebook containing the code used for data analysis is available at Zenodo\cite{kratz_researcher_2014-1}.
