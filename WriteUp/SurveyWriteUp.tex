...
%try looking at some NCEAS papers about meta-analysis, which is only possible if people share. NCEAS often publishes stuff about the big impact of their work and how it's only possible with data sharing
...
%depends on where the data goes
...
Quality assurance of published data facilitates reuse, but only if potential reusers trust the means of assessment.
...
In addition to reuse, data publication is meant to facilitate award of credit to researchers who create useful datasets.
To that end, we asked what indicators researchers would most respect when evaluating a dataset's impact.
...
The prestige of a data publication depends not only on quality and impact, but also on format.
We distilled a multiplicity of data publications formats to four generic models–- with or without a data paper and with or without peer review-- and asked respondents how heavily they would weight each model if they were evaluating a colleague's curriculum vit{\ae}.
As a point of comparison, we also asked about the value of a traditional paper; 60\% ($n=145$) give one a great deal of weight and another 36\% ($n=87$) give it significant weight.
