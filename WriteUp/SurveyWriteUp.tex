...
We heard from researchers accross the academic career spectrum: 41\% ($n=102$) were principal investigators/lab heads, 24\% ($n=61$) postdocs, and 16\% ($n=41$) grad students.
...
We decomposed the prevalent models of data publication into a set of potential features and asked which would distinguish a ``published'' dataset from a ``shared'' one (Figure 4A).
The most common expectations relate to access: 68\% ($n=166$) expect a published dataset to be openly available and 54\% ($n=133$) expect it to have been deposited in a repository.
Substantially researchers expected a published dataset to accompanied by a traditional publication ($43\%, n=105$) than a data paper ($22\%, n=55$).
Only a 29\% ($n=70$) minority expect published data to have been peer reviewed.

To find out what researchers expected from data peer review, we asked what they would expect peer review of data to entail and provided a list of considerations that various organizations take into account when evaluating/validating datasets (Figure 4B).
