...
%DIF DEL SurveyPaperV1.tex   Thu Dec 11 09:03:08 2014
%DIF ADD SurveyWriteUp.tex   Thu Dec 11 09:30:03 2014
%DIF < Skip to content
%DIF <  This repository
%DIF < Explore
%DIF < Gist
...
For \DIFaddbegin \DIFadd{significance testing, we consolidated subdisciplines into 8 high-level disciplines.
...
We distilled a multiplicity of data publications formats to four generic modelsâ€“- with or without a data paper and with or without peer review-- and asked respondents how much each would contribute to a \DIFdelbegin \DIFdel{reseacher}\DIFdelend \DIFaddbegin \DIFadd{researcher}\DIFaddend 's curriculum vit{\ae} (Figure \ref{fig:values}D).
...
\DIFaddend Whereas DataONE uncovered statistically distinct data sharing attitudes between respondents in different \DIFdelbegin \DIFdel{roles and }\DIFdelend disciplines, we did not.
...
This survey should have 80\% sensitivity to an effect size at the top of this range, $\Phi_{C}=0.17$, so we find it plausible that the detection of }\DIFaddend statistically meaningful distinctions in one survey and not the other \DIFdelbegin \DIFdel{is at least as likely to be }\DIFdelend \DIFaddbegin \DIFadd{could }\DIFaddend an artifact of the difference in \DIFdelbegin \DIFdel{$n$ (1329 vs. 249) as }\DIFdelend \DIFaddbegin \DIFadd{statistical power (from an $n$ of $1329$ vs. $249$) rather than }\DIFaddend a reflection of real differences in the respondent populations.
\DIFaddbegin \DIFadd{However, $0.17$ is comfortably a ``small'' effect, so we are unlikely here to have missed large or even moderate effects by chance\mbox{%DIFAUXCMD
\cite{cohen_statistical_1988}
}%DIFAUXCMD
.
}\DIFaddend 
...
Questions that related to US policies were analyzed based on US respondents only; one questions about the NIH was analyzed based only on US biologists\ref{fig:policy_knowledge}}\DIFaddend .
...
To facilitate comparison, $\chi^{2}$ effects were measured as Cram\'er's V ($\Phi_{C}$)\mbox{%DIFAUXCMD
...
Edges depict relationship strength \DIFdelbeginFL \DIFdelFL{; line thickness is proportional to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{as measured by }\DIFaddendFL odds ratio.  
