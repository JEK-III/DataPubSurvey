...
Blog
Help
John Ernest Kratz JEK-III
 
2  Unwatch 
  Star 0
 Fork 1JEK-III/DataPubSurvey
 tree: e9fd404c65  DataPubSurvey/WriteUp/SurveyWriteUp.tex
John Kratzqb-x on Aug 18 Spiff up policy awareness script.
2 contributors John KratzJohn Ernest Kratz
604 lines (486 sloc)  56.26 kb RawBlameHistory  
% Template for PLoS
...
More recently, Callaghan et al. (2012) suggested distinguishing between shared (available), \underline{p}ublished (also citable), and \underline{P}ublished (also peer-reviewed) data \cite{callaghan_making_2012}.
We have argued that the consensus in the scholarly communications community-- publishers, librarians, curators-- is that published data is openly available and citable, but that what kind of validation (if any) is required to qualify is still an open question \cite{kratz_data_2014}.
...
We saw surprisingly few significant differences in responses between disciplines or roles, so we have presented the results in aggregate.
...
The predominance of direct contact is problably in part an artifact of awareness: respondents necessarily know when they give someone their data directly, but they may not be notified when someone downloads it from a repository or website.
...
Because previous studies reported differences in citation practices between disciplines \cite{swan_share_2008, harley_assessing_2010, tenopir_data_2011}, we tested whether discipline had a detectable effect on the response  (omitting Mathematics because of low $n$) and found that it did not ($\chi^{2}=15.6$, $p=0.9$).
...
The strongest relationship, between less satisfaction with past acknowledgment and belief that authorship would be appropriate, was still far from statistically meaningful ($\chi^{2}= 3.26$, $p= 0.20$, corrected $\alpha= 0.0125$).
...
To learn whether respondents selected data publication features or peer-review assessments independently or as coherent constellations of ideas, we tested for independence of items within each question. 
